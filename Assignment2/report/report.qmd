...
There was no augmentation used for the RF model aside from zoom to fit the desired size requirements. The CNN model was prepared without augmentation as well. My reasoning is that conducting other augmentations could ruin the mask encodings and the augmentations may not be consistent (i.e the same flip may not be applied to the image and mask). Finally, all the data was stored in a `.npz` file to keep data consistent and easily loaded into other files. 
...
![CNN History](../src/vgg_model_loss_accuracy.png){width=350px}
...
|:-------------------------------------------------------:|:-------------------------------------------------------:|
| ![Saliency Map](../src/cnn_saliency_map.png){width=300px}    | ![GradCAM Map](../src/cnn_gradcam_2.png){width=300px}     |
